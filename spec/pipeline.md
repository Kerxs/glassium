...
一块面板一个 `Panel` 结构体（304 字节），按 **512 字节**的步长（两个 256 字节的对齐单位）排进一条 uniform buffer，
每次 draw 只换动态偏移。一个合并组一个 `Group`（1232 字节），按 **1280 字节**步长排。
...
那个形状的覆盖率（`shapeBox`，没有时恰好是 1）。两者都用「到四条边的距离」算带圆角盒子的 SDF：
...
填充一块一个 `Fill`（432 字节，512 字节步长）：`rect`、`radii`、`color`（未预乘的 rgb + alpha，alpha 已乘
...
Panel 的裁剪后半截相同。另有一个 16 字节的 `Dest`（画到哪里）：`scale`（一个目标像素是几个画布设备像素：场景目标是
画布 ÷ 场景，画布本身是 1）、`aa`（抗锯齿过渡的宽度 = 一个目标像素）、`linear`。片元：`px = 片元位置 · scale`，形状与
面板一样在画布设备像素里算，`a = color.a · clamp(0.5 − sd / aa) · clamp(0.5 − max(交集矩形的 sd, 单独算的形状的 sd) / aa)`，
输出预乘色。
