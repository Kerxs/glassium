...
要么退回逐面板模糊，要么接受近似，必须显式决定。它成立还要求两边在同一色彩空间：默认两边都在 sRGB
编码空间；线性光模式（`blendSpace: 'linear'`）下两边都在线性光里 —— 模糊链存 sRGB 编码、采样时先解码，
tint 先换成线性值，调色之后再编码回 sRGB。
...
一块面板一个 `Panel` 结构体（416 字节），按 **512 字节**的步长（两个 256 字节的对齐单位）排进一条 uniform buffer，
每次 draw 只换动态偏移。一个合并组一个 `Group`（1680 字节），按 **1792 字节**步长排。
...
| 176 | `clipRadiiY: vec4` | 可见区域四角的竖直半径（与 `clipRadii` 相等的角是圆角） |
| 192 | `clipInv: array<vec4, 2>` | 1 ÷ 水平半径、1 ÷ 竖直半径（半径 0 写 0）—— 椭圆角用，着色器里不除以 uniform |
| 224 | `shapeBox: vec4` | 单独算的那个圆角形状的矩形（被截断的圆角祖先、clip-path 的圆 / 椭圆）；没有时写 ±65536 |
| 240 | `shapeRadii: vec4` | 它的四角水平半径（没有时 0） |
| 256 | `shapeRadiiY: vec4` | 它的四角竖直半径 |
| 272 | `shapeInv: array<vec4, 2>` | 它的 1 ÷ 半径 |
| 304 | `maskPaint: vec4` | 遮罩（mask-image 的渐变）：种类（0 没有 · 1 线性 · 2 径向）、色标数、重复、空 |
| 320 | `maskGeom: vec4` | 线性：起点、(终点 − 起点) ÷ 长度²；径向：中心、1/rx、1/ry（画布设备像素，绝对坐标） |
| 336 | `maskAlpha: array<vec4, 2>` | 色标的不透明度（5 个；luminance 时已乘亮度） |
| 368 | `maskAt: array<vec4, 2>` | 色标的位置（5 个）；`maskAt[1].y`、`z` 是重复的周期的倒数与周期 |
| 400 | `maskSpan: vec4` | 相邻两个位置之差的倒数（重合的是 0，硬边） |

`Group` = 16 字节的头（成员数、k（画布设备像素）、debugMode、空）+ 4 个紧挨着的 `Panel`。

片元的覆盖率 = 形状的覆盖率 × 裁剪区域的覆盖率。裁剪区域的覆盖率 = 交集矩形（带角上的圆角）的覆盖率 × 单独算的
那个形状的覆盖率（`shapeBox`，没有时恰好是 1）× 遮罩的不透明度（按 `maskPaint` 求渐变位置 t、在相邻两个色标的
不透明度之间线性插值 —— 与填充的渐变同一套取色标的办法；没有遮罩时恰好是 1）。两者都用「到四条边的距离」算带圆角盒子的 SDF：
`o = (max(x0 − p.x, p.x − x1), max(y0 − p.y, p.y − y1))`，按像素落在哪个象限取角的两个半径 rx、ry。两个相等（圆角）时
`e = o + r`，`sd = |max(e, 0)| + min(max(e.x, e.y), 0) − r`；不相等（椭圆角）时 `q = o + (rx, ry)`，q 两个分量都大于 0
（在角的那一块里）的话 `k = q ⊙ (1/rx, 1/ry)`，`sd = (|k| − 1)·|k| / |k ⊙ (1/rx, 1/ry)|`（椭圆隐函数除以梯度长度，
边界附近准），否则 `sd = max(q.x − rx, q.y − ry)`。不用「到中心的偏移减半宽」：区域一边是 ±65536 时中心在几万像素
之外，f32 在那个量级上的精度不够；到边的距离是精确的。没有裁剪时覆盖率恰好是 1.0，乘上去逐位不变。合并组取成员
裁剪覆盖率的最大值。探针不乘裁剪 —— 它验的是光学。

按压处的光加在亮边的加性光上：`lit = 高光项 + strength · exp(−|p − c|² / 2σ²)`。强度为 0 时加的是 0，逐位不变。
合并组把成员各自的光斑相加（只有被按下的那块有光）。

填充一块一个 `Fill`（544 字节，768 字节步长）：`rect`、`radii`、`color`（未预乘的 rgb + alpha，alpha 已乘
CSS 上的不透明度）、`clip`、`clipRadii`、`pose`，含义与 Panel 的同名字段相同；`paint`、`geom`、`stops[5]`、`at[2]`、
`span` 是渐变（fill.wgsl.ts 的注释逐项写了）；`radiiY`、`inv[2]` 是自己的椭圆角；最后 `clipRadiiY` … `shapeInv` 与
Panel 的裁剪后半截相同，再后面 `maskPaint` … `maskSpan` 是遮罩。另有一个 16 字节的 `Dest`（画到哪里）：`scale`（一个目标像素是几个画布设备像素：场景目标是
画布 ÷ 场景，画布本身是 1）、`aa`（抗锯齿过渡的宽度 = 一个目标像素）、`linear`。片元：`px = 片元位置 · scale`，形状与
面板一样在画布设备像素里算，`a = color.a · clamp(0.5 − sd / aa) · clamp(0.5 − max(交集矩形的 sd, 单独算的形状的 sd) / aa) · 遮罩`，
输出预乘色。
