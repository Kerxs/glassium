...
### 3. 高光：对称 `abs()` → 不对称，且新增暗边（0.1.0 起撤回：改回双面，再加侧面的基础亮度）

**0.1.0 起这一条作废。** 对着 iOS 26 真机截图量（docs/calibration.md「质感对照」），Apple 的亮边就是**双面**的：
圆按钮上下两条 +68 / +73、左右 +36 / +30，正在播放面板四条边 +39..+68，哪里都没有暗边。所以改回上游的 `abs()`，
再加一个 base（左右不是 0，而是上下的一小半）：`base + (1 − base)·|n·L|^gloss`，L 竖直，base 0.45。暗边项去掉。
下面是 0.0.1 时的理由，留作记录：
