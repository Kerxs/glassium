...
### 裁剪：矩形跟上了，圆角与 clip-path 没有

玻璃画在底下的画布上，DOM 的裁剪管不到它。所以 Glassium 自己找出每块面板的**裁剪祖先**
（`overflow` 不是 visible、或 `contain: paint` 的祖先），每帧量它们的 padding box，与面板的
scissor 求交（`src/renderer/clipping.ts`）。面板在滚动容器里被滚出可见区域时，玻璃跟着被裁掉。
...
- 裁剪矩形是**矩形**。裁剪祖先带 border-radius 时，圆角外那一小块玻璃不会被裁掉。
- `clip-path`、`mask` 不管。
