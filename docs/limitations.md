...
`overflow: hidden / auto / scroll` 与 `clip-path` 只裁 DOM。面板在一个滚动容器里被滚出
容器的可见区域、但还在视口里时，DOM 被裁掉了，**玻璃还画在原处**。

要修得在每帧的测量里找出面板的裁剪祖先、把它们的矩形与面板的裁剪矩形求交（圆角裁剪只能近似）。
不难，但第一期没做。在那之前，别把玻璃面板放进会滚动的容器里，或者让容器不裁剪。
